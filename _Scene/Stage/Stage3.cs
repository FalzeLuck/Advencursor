        ...
        public class Stage3 : StageAbstract
        {
            ...
            private void EnemyManage()
            {
                ...
                if (boss_spawn_time > 120f && !boss_spawned)
                {
                    startWarning = false;
                    elite_spawn_time = 0f;
                    boss_obj = new Boss3(Boss3Texture, new Vector2(Globals.Bounds.X / 2, -200), 600000, 5000, 6, 12)
                    {
                        movementAI = new FollowMovementAI()
                        {
                            target = player,
                ...
                {
                    tomatoBoss = new Boss1(Boss1Texture, new(100000, 500), health: Int32.MaxValue, attack: 3000, row: 3, column: 8)
                    {
                        movementAI = new FollowMovementAI
                        {
                            target = player,
